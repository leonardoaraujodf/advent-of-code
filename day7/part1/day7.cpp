...
static const std::string cards = "AKQJT98765432";
...
public:
    Hand(std::string v, unsigned int b) : m_val(v), m_bid(b), m_score2(5,0)
    {
    ...
    void set_hand_information(std::string &hand) {
        ...
        m_score1 = 0;
        for (unsigned int i = 0; i < cards.size() && num_cards_found < 5; i++)
        {
            char c = cards[i];
            unsigned int partial_score = 0;
            std::size_t found = 0;
