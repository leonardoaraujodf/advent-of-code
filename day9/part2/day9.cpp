...
static void extrapolate_values_right(VEC_LL &in, std::vector<VEC_LL> &diffs)
{
    long long new_val;
...
static void extrapolate_values_left(VEC_LL &in, std::vector<VEC_LL> &diffs)
{
    long long new_val;
    auto it = diffs[diffs.size() - 1].begin();
    diffs[diffs.size() - 1].emplace(it, 0);
    for (unsigned int i = diffs.size() - 1; i > 0; --i)
    {
        new_val = diffs[i - 1].front() - diffs[i].front();
        it = diffs[i - 1].begin();
        diffs[i - 1].emplace(it, new_val);
    }
    new_val = in.front() - diffs[0].front();
    it = in.begin();
    in.emplace(it, new_val);
...
static long long predict_next_val(VEC_LL &h)
{
    ...
    extrapolate_values_left(h, diffs);
    return h.front();
}
